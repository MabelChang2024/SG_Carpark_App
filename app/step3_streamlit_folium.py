...
import folium
from utils import *   #* means that you import all functions 
...
with st.expander("Shopping Mall"):   #expander allows you to open and close certain areas 
...
if len(filter_malls) != 0:
    for index in range(len(malls)):
        ...
        if malls[index][0] not in filter_malls:   #when there is no selection 
            pass
        else:
